...
export function CurriculumsActionDialog() {
  ...
  const { createCurriculum, updateCurriculum, setOpenedDialog, currentRow } =
  ...
  async function onSubmit(data: FormData) {
    try {
      if (isEdit && currentRow) {
        ...
        toast.success(t('curriculums.actionDialog.updateSuccess'));
      } else {
        ...
        toast.success(t('curriculums.actionDialog.createSuccess'));
      }
      ...
      toast.error(
        isEdit
          ? t('curriculums.actionDialog.updateError')
          : t('curriculums.actionDialog.createError')
      );
    }
  }
      ...
      <Dialog open onOpenChange={handleDialogClose}>
        <DialogContent className="xs:max-w-[80%] max-h-[90vh] sm:max-w-[70%]">
          <DialogHeader>
            <DialogTitle>
              {isEdit
                ? t('curriculums.actionDialog.editTitle')
                : t('curriculums.actionDialog.createTitle')}
            </DialogTitle>
            <DialogDescription>
              {isEdit
                ? t('curriculums.actionDialog.editDescription')
                : t('curriculums.actionDialog.createDescription')}
            </DialogDescription>
          </DialogHeader>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel required>
                      {t('curriculums.actionDialog.fields.subject')}
                    </FormLabel>
                    <FormControl>
                      <CreatableCombobox
                        ...
                        placeholder={t(
                          'curriculums.actionDialog.selectSubject'
                        )}
                        searchPlaceholder={t(
                          'curriculums.actionDialog.searchSubjects'
                        )}
                        emptyText={t(
                          'curriculums.actionDialog.noSubjectsFound'
                        )}
                        createText="curriculums.actionDialog.createNewSubject"
                      />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel required>
                      {t('curriculums.actionDialog.fields.name')}
                    </FormLabel>
                    <FormControl>
                      <Input
                        {...field}
                        placeholder={t(
                          'curriculums.actionDialog.placeholders.name'
                        )}
                      />
                    </FormControl>
                    <FormMessage />
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      {t('curriculums.actionDialog.fields.description')}
                    </FormLabel>
                    <FormControl>
                      <Textarea
                        {...field}
                        rows={3}
                        placeholder={t(
                          'curriculums.actionDialog.placeholders.description'
                        )}
                      />
                    </FormControl>
                    <FormMessage />
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <FormLabel className="text-base font-medium">
                    {t('curriculums.actionDialog.fields.lessons')}
                  </FormLabel>
                  <Button
                    type="button"
                    variant="outline"
                                      ...
                                      render={({ field }) => (
                                        <FormItem>
                                          <div className="flex flex-col sm:flex-row">
                                            ...
                                            >
                                              {t(
                                                'curriculums.actionDialog.fields.lessonName'
                                              )}
                                            </FormLabel>
                                            <FormControl className="flex-1">
                                              <Input
                                                ...
                                                className="h-8"
                                                placeholder={t(
                                                  'curriculums.actionDialog.placeholders.lessonName'
                                                )}
                                              />
                                            </FormControl>
                                          </div>
