...
type TeacherForm = z.infer<ReturnType<typeof getFormSchema>>;

export function TeachersActionDialog() {
  const { t } = useTranslation();
  const { createTeacher, updateTeacher, currentRow, setOpenedDialog } =
    useTeachersContext();
  ...
  const defaultValues: TeacherForm = isEdit
    ? {
  ...
  const subjectOptions = useMemo(() => {
    return (subjectsQuery.data?.subjects || []).map((subject) => ({
      label: subject.name,
      value: subject.id,
    }));
  ...
  async function onSubmit(values: TeacherForm) {
    try {
      if (isEdit) {
        ...
        updateTeacher(response as unknown as User);
      } else {
        const createData = {
          ...values,
        ...
        toast.success(t('dialogs.action.success.createTeacher'));
        createTeacher(response as unknown as User);
      }
    } catch {}
      ...
      <div className="border-border border-b pb-6">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div className={'col-span-2 grid grid-cols-2 gap-2'}>
            ...
            />
            {/* show stae only if country is US */}
      ...
      <div className="border-border border-b pb-4">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          {status === UserStatus.SUSPENDED && (
            <>
              {form.getValues('statusUpdatedAt') && (
                <FormField
                  control={form.control}
                  name="statusUpdatedAt"
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t('common.statusUpdatedAt')}</FormLabel>
                      <FormControl>
                        <Input
                          value={
                            field.value
                          ...
                          readOnly
                          placeholder={t('common.noDateSelected')}
                        />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
      ...
      <Dialog open onOpenChange={handleDialogClose}>
        <DialogContent className="xs:max-w-[80%] sm:max-w-[70%]">
          <DialogHeader className="mb-8 text-left">
            <DialogTitle>
              {isEdit
                ? t('dialogs.action.editTitleTeacher')
                : t('dialogs.action.addTitleTeacher')}
            </DialogTitle>
            <DialogDescription>
              {isEdit
                ? t('dialogs.action.editDescriptionTeacher')
                : t('dialogs.action.addDescriptionTeacher')}
            </DialogDescription>
          </DialogHeader>
          <div className="mb-4">
            <Form {...form}>
