import { SystemRole } from '@edusama/common';
...
import { User } from '@/lib/trpc';
import { getStatusBadgeVariant } from '@/utils';

export function TeachersViewDialog() {
  const { t } = useTranslation();
  const { currentRow, setOpenedDialog } = useTeachersContext();
        ...
        <DialogHeader>
          <DialogTitle>
            {t('dialogs.view.title', {
              entity: t('sidebar.navigation.teachers'),
            })}
          </DialogTitle>
          ...
          <div className="space-y-6">
                  ...
                  {currentRow.roles.map((role) => {
                    return (
                      <Badge variant="outline" className="capitalize">
                        {role.role.code
                          ? t(`systemRoles.${role.role.code as SystemRole}`)
                          : role.role.name}
                      </Badge>
                    );
                  })}
            ...
            {currentRow.about && (
              <>
                ...
                <div>
                  <h4 className="mb-3 text-sm font-medium">
                    {t('dialogs.view.fields.about')}
                  </h4>
                  <p className="text-sm">{currentRow.about}</p>
                </div>
              </>
            )}
