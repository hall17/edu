...
import { TeachersDataTableToolbar } from './TeachersDataTableToolbar';
...
export function TeachersTable() {
  ...
  const { teachersQuery, filters, setFilters } = useTeachersContext();
  ...
  return (
    <CustomTable
      data={teachersQuery.data?.users ?? []}
      rowCount={teachersQuery.data?.count ?? 0}
      columns={columns}
      filters={filters}
      ...
      CustomToolbar={TeachersDataTableToolbar}
    />
  );
}
...
export function useColumns(): ColumnDef<User>[] {
  const { t } = useTranslation();
  const { setOpenedDialog, setCurrentRow } = useTeachersContext();
  return [
    ...
    },
    {
      ...
      cell: ({ row }) => <TeachersStatusCell row={row} />,
...
interface TeachersStatusCellProps {
  row: Row<User>;
}

function TeachersStatusCell({ row }: TeachersStatusCellProps) {
  const { t } = useTranslation();
  const { teachersQuery } = useTeachersContext();
  ...
  const updateStatusMutation = useMutation(
    trpc.user.update.mutationOptions({
      onSuccess: () => {
        ...
        toast.success(t('teachers.updateStatusSuccess'));
        setOpen(false);
      },
      onError: () => {
        toast.error(t('teachers.updateStatusError'));
      },
    })
  );
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            <h4 className="leading-none font-medium">
              {t('teachers.statusDialog.title')}
            </h4>
            <p className="text-muted-foreground text-sm">
              {t('teachers.table.statusChangeWarning')}
            </p>
          </div>

          <div className="space-y-2">
            <p className="text-sm font-medium">
              {t('teachers.statusDialog.currentStatus')}
            </p>
          ...
          <div className="space-y-3">
            <p className="text-sm font-medium">
              {t('teachers.statusDialog.newStatus')}
            </p>
            <Select
              value={status}
