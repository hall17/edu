...
function ClassroomCalendarTableViewContent() {
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            <CardTitle>{t('classrooms.calendar.title')}</CardTitle>
            <CardDescription>
              {t('classrooms.calendar.description')}
            </CardDescription>
          </div>
          <Button onClick={() => setOpenedDialog('create')} className="ml-4">
            <Plus className="mr-2 size-4" />
            {t('classrooms.calendar.addNew')}
          </Button>
        </div>
        ...
        <div className="flex flex-col gap-4 pt-4">
          <div className="flex flex-col items-center gap-4 sm:flex-row">
            {/* Search Input */}
            <SearchInput
              placeholder={t('classrooms.calendar.search.placeholder')}
            ...
            <MultiSelect
              className="!w-[250px]"
              placeholder={t('classrooms.calendar.filters.subject')}
              options={
          ...
          <CardContent>
            {classroomIntegrationSessions?.length === 0 ? (
              <div className="text-muted-foreground py-8 text-center">
                {t('classrooms.calendar.noSessionsFound')}
              </div>
            ) : (
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex-1 space-y-3">
                              ...
                              <div className="space-y-2">
                                {session.teacher && (
                                  <div className="flex items-center gap-2">
                                    <span className="text-muted-foreground text-sm font-medium">
                                      {t('classrooms.calendar.labels.teacher')}:
                                    </span>
                                    <Badge
                                      variant="secondary"
                                      className="text-xs"
                                ...
                                {session.classroomIntegration?.subject && (
                                  <div className="flex items-center gap-2">
                                    <span className="text-muted-foreground text-sm font-medium">
                                      {t('classrooms.calendar.labels.subject')}:
                                    </span>
                                ...
                                {session.lessons &&
                                  session.lessons.length > 0 && (
                                    <div className="flex items-center gap-2">
                                      <span className="text-muted-foreground text-sm font-medium">
                                        {t(
                                          'classrooms.calendar.labels.lessons'
                                        )}
                                        :
                                      </span>
                                      <div className="flex flex-wrap gap-1">
                                        {session.lessons.map((lessonItem) => (
                                          <Badge
                              ...
                              {totalStudents > 0 ? (
                                  ...
                                  <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                      ...
                                      <span className="text-sm font-medium">
                                        {t(
                                          'classrooms.calendar.labels.attendance'
                                        )}
                                      </span>
                                    </div>
                                    <span className="text-muted-foreground text-sm">
                                      {presentStudents}/{totalStudents}{' '}
                                      {t('classrooms.calendar.labels.students')}{' '}
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-muted-foreground text-sm">
                                    {t(
                                      'classrooms.calendar.labels.noAttendanceRecord'
                                    )}
                                  </span>
                                </div>
                              )}
                              ...
                              {session.description && (
                                <div className="border-t pt-2">
                                  <div className="text-muted-foreground mb-1 text-sm font-medium">
                                    {t(
                                      'classrooms.calendar.labels.description'
                                    )}
                                    :
                                  </div>
                                  <p className="text-muted-foreground text-sm">
                            ...
                            <div className="ml-4 flex gap-2">
                              ...
                              >
                                ...
                                {t('classrooms.calendar.actions.view')}
                              </Button>
                              <Button
                                variant="outline"
                                size="sm"
                              ...
                              >
                                ...
                                {t('classrooms.calendar.actions.edit')}
                              </Button>
                            </div>
                          </div>
                        </CardContent>
...
export function ClassroomCalendarTableView() {
  return (
    <ClassroomCalendarProvider>
      <ClassroomCalendarTableViewContent />
    </ClassroomCalendarProvider>
  );
}
