...
export function DataTableFacetedFilter<TData, TValue>(
  ...
  const facets = props.facets ?? column?.getFacetedUniqueValues();
  const selectedValues = new Set(
    props.selectedValues ?? (column?.getFilterValue() as string[])
  );
    ...
    <Popover>
      ...
      <PopoverContent className="w-[200px] p-0" align="start">
        <Command>
          ...
          <CommandList>
            <CommandEmpty>{t('table.filters.noResults')}</CommandEmpty>
            <CommandGroup>
              {options.map((option) => {
